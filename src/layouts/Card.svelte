...
<figure
  ...
  <figcaption class="p-6">
    <h3 class="mb-1 font-light text-primary">{name}</h3>
      {#if username}<a class="no-underline rounded-lg github" href={`https://github.com/${username}/${name}`}>Github 🎩&rarr;</a>{/if}
  </figcaption>
</figure>

<style>
  figure:hover .relative p {
    opacity: 1;
  }
  figure:hover img {
    opacity: 0.2;
